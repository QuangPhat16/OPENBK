...
const CourseController = {
  async createCourse(req, res) {
    try {
      ...
      if (checkNull({ authorID, courseName, imageUrl: filePath, category, description, price })) return res.status(400).json({ message: 'Course creation failed, some fields are missing' })
  ...
  async updateCourse(req, res) {
    try {
      ...
      const updated = await Course.update(params, {
        where: {
          courseID,
        }
      })
